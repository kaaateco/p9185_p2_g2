...
cbsa_pcthwite_summary <- tract %>%
  group_by(`cbsa_id`) %>%
  summarise(mean_pctwhite = mean(pct_white, na.rm = TRUE)) |> 
  drop_na()

cbsa_map_data <- coords_df |> 
  mutate(`CBSA` = as.numeric(`CBSA`)) |> 
  right_join(cbsa_pcthwite_summary, by = c("CBSA" = "cbsa_id"))

ggplot(cbsa_map_data, aes(`X`, `Y`, group = CBSA, fill = mean_pctwhite)) +
  geom_polygon(color = "white", size = 0.2) +
  scale_fill_gradient(low = "blue", high = "red") +
...
```{r}
ggplot(tract, aes(x = ruca_agg, y = dens_v2, fill = ruca_agg)) +
  geom_boxplot() +
  theme_classic() +
...
nested_mod <- lmerTest::lmer(pm25 ~ pct_white*pct_pov 
                             + ruca_agg + dens_v2 + (1 | state_abb/cbsa_id),
                     data = tract)
...
<!-- &\ \ \ \ \ \ \ \  \beta_{3} \text{(Area = Urban)} + \beta_4 \text{Density} + \beta_5 \text{(PCT_WHITE x PCT_POV)} + b_{j(k)}{CSBA(State)},\\ -->
